...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_element'])) {
    $elementu_izena = mysqli_real_escape_string($conn, $_POST['elementu_izena']);
    $check = mysqli_query($conn, "SELECT * FROM usuario_elementuak WHERE usuario_id = {$_SESSION['user_id']} AND elementu_izena = '$elementu_izena'");
    
    if (mysqli_num_rows($check) > 0) {
        mysqli_query($conn, "DELETE FROM usuario_elementuak WHERE usuario_id = {$_SESSION['user_id']} AND elementu_izena = '$elementu_izena'");
        $_SESSION['success'] = "Elementua kendu da!";
    }
    header('Location: elements.php');
    exit;
}
...
<body>
                ...
                <div class="form-group">
                    ...
                    <select name="elementu_izena" required>
                        ...
                        <?php while ($el = mysqli_fetch_assoc($all_elements)): ?>
                            <option value="<?= htmlspecialchars($el['izena'], ENT_QUOTES, 'UTF-8') ?>">
                                <?= htmlspecialchars($el['izena'], ENT_QUOTES, 'UTF-8') ?> (<?= htmlspecialchars($el['mota'], ENT_QUOTES, 'UTF-8') ?>)
                            </option>
                        <?php endwhile; ?>
                    </select>
                </div>
            ...
            <?php if (mysqli_num_rows($result) > 0): ?>
                <?php while ($el = mysqli_fetch_assoc($result)): ?>
                    ...
                    <div class="element-card">
                        <h3><?= htmlspecialchars($el['izena'], ENT_QUOTES, 'UTF-8') ?></h3>
                        <p><strong>Mota:</strong> <?= htmlspecialchars($el['mota'], ENT_QUOTES, 'UTF-8') ?></p>
                        <p><strong>Bizitza:</strong> <?= intval($el['bizitza']) ?></p>
                        <p><strong>Erasoa:</strong> <?= intval($el['erasoa']) ?></p>
                        <p><strong>Defentsa:</strong> <?= intval($el['defentsa']) ?></p>
                        <button class="btn btn-edit" onclick='editElement(<?= $json ?>)'>✏️ Editatu</button>
                        <button class="btn btn-danger" onclick='deleteElement("<?= htmlspecialchars($el['izena'], ENT_QUOTES, 'UTF-8') ?>")'>🗑️ Kendu</button>
                    </div>
                <?php endwhile; ?>
            <?php else: ?>
    ...
    <div id="deleteModal" class="modal">
        <div class="modal-content">
            ...
            <h2>Konfirmatu ezabaketa</h2>
            <p>Ziur al zaude elementu hau kendu nahi duzula?</p>
            <form method="POST" id="deleteForm">
                <input type="hidden" name="elementu_izena" id="delete_element_izena">
                <button type="submit" name="delete_element" class="btn btn-danger">Bai, kendu</button>
                <button type="button" class="btn btn-secondary" onclick="closeDeleteModal()">Utzi</button>
            </form>
        </div>
    </div>
