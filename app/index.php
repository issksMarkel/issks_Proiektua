...
// CORRECCIÓN: Usar 'home' como valor por defecto
$route = $_GET['route'] ?? 'home';

// CORRECCIÓN: Cambiar todas las rutas para que coincidan
if ($route === 'home') {
    // Página principal
    ?>
    <!DOCTYPE html>
...
elseif ($route === 'register') {
    ...
    <body>
        <div class="form-container">
            ...
            <a href="/?route=home" class="back-btn">← Hasierara itzuli</a>
            ...
            if (isset($_GET['error'])) {
                $errores = [
                    ...
                    'nan_existe' => 'NAN hau jada erregistratuta dago'
                ];
                $error = $_GET['error'];
            ...
            <!-- CORRECCIÓN: Action del formulario corregido -->
            <form action="/?route=procesar-registro" method="POST" onsubmit="return validatuFormularioa()">
                <!-- Izen abizenak -->
        ...
        <script>
            ...
            document.getElementById('telefonoa').addEventListener('blur', function() {
                const telefonoa = this.value;
                const pattern = /^[0-9]{9}$/;
                if (telefonoa && !pattern.test(telefonoa)) {
                    this.style.borderColor = '#dc3545';
                } else if (telefonoa) {
                    this.style.borderColor = '#28a745';
                }
...
elseif ($route === 'procesar-registro') {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($errores)) {
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                header('Location: /?route=register&error=nan_existe&izena=' . urlencode($izena) . '&telefonoa=' . urlencode($telefonoa) . '&jaiotze_data=' . urlencode($jaiotze_data) . '&email=' . urlencode($email));
                exit;
            } else {
                ...
                if (mysqli_query($conn, $insert_query)) {
                    // Éxito
                    header('Location: /?route=register&exito=1');
                    exit;
                } else {
                    // Error en la inserción
                    header('Location: /?route=register&error=general&izena=' . urlencode($izena) . '&nan=' . urlencode($nan) . '&telefonoa=' . urlencode($telefonoa) . '&jaiotze_data=' . urlencode($jaiotze_data) . '&email=' . urlencode($email));
                    exit;
                }
            }
        } else {
            ...
            header('Location: /?route=register&' . $params);
            exit;
        }
    } else {
        // Método no permitido
...
elseif ($route === 'usuarios') {
    ...
    <body>
        <div class="error-container">
            ...
            <a href="/?route=home" class="btn">Volver a la página principal</a>
        </div>
    </body>
    </html>
    <?php
